...
export function registerPsaTools(server) {
    // ─── List PSA Clients ─────────────────────────────────────────────────────
    server.registerTool("keka_list_psa_clients", {
...
Use returned IDs with keka_list_psa_projects to find projects for a client.`,
        ...
        try {
            ...
            const res = await client.getPaginated("/psa/clients", {
            ...
            const lines = [
                ...
                `|---|---|---|---|---|---|`,
                ...res.data.map((c) => `| ${c.name} | ${c.id} | ${c.email ?? "—"} | ${c.phone ?? "—"} | ` +
    ...
    // ─── List PSA Projects ────────────────────────────────────────────────────
    server.registerTool("keka_list_psa_projects", {
...
Returns: PSA projects with ID, name, client, status, start/end dates, budget, currency, and project manager.`,
        ...
        try {
            const client = getKekaClient();
            const res = await client.getPaginated("/psa/projects", {
                pageNumber: params.pageNumber,
                pageSize: params.pageSize,
            });
            if (!res.succeeded) {
