...
export function registerLeaveTools(server) {
    // ─── List Leave Types ─────────────────────────────────────────────────────
    server.registerTool("keka_list_leave_types", {
...
Use the returned IDs when creating leave requests with keka_create_leave_request.`,
        ...
        try {
            ...
            const res = await client.getPaginated("/time/leavetypes", {
            ...
            const lines = [
                ...
                `| Name | Code | ID | Paid? |`,
                `|---|---|---|---|`,
                ...res.data.map((lt) => `| ${lt.name} | ${lt.code ?? "—"} | ${lt.id} | ${lt.isPaid ? "Yes" : "No"} |`),
            ];
    ...
    // ─── List Leave Requests ──────────────────────────────────────────────────
    server.registerTool("keka_list_leave_requests", {
...
Examples:
  ...
  - See who is on leave this month → from='2025-03-01', to='2025-03-31'`,
        ...
        try {
            ...
            const res = await client.getPaginated("/time/leaverequests", queryParams);
            if (!res.succeeded) {
                return { content: [{ type: "text", text: `Error: ${res.message}` }] };
            }
            if (!res.data.length) {
            ...
            for (const lr of res.data) {
                lines.push(`- **${lr.employeeName ?? lr.employeeId}** | ${lr.leaveType?.name ?? "—"} | ` +
    ...
    server.registerTool("keka_create_leave_request", {
...
Note: Requires the API key to have leave management write permissions.`,
        ...
        try {
            ...
            if (params.requestedBy)
                body.requestedBy = params.requestedBy;
            const res = await client.post("/time/leaverequests", body);
            if (!res.succeeded) {
                return { content: [{ type: "text", text: `Error: ${res.message}` }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
    ...
    // ─── Get Leave Balances ───────────────────────────────────────────────────
    server.registerTool("keka_get_leave_balances", {
...
Returns: Leave balance breakdown per employee and leave type — opening, earned, taken, pending, and closing balances.`,
        ...
        try {
            ...
            if (params.employeeIds)
                queryParams.employeeIds = params.employeeIds;
            const res = await client.getPaginated("/time/leavebalance", queryParams);
            if (!res.succeeded) {
                return { content: [{ type: "text", text: `Error: ${res.message}` }] };
            }
            if (params.response_format === ResponseFormat.JSON) {
            ...
            const lines = [
                ...
                `|---|---|---|---|---|---|---|`,
                ...res.data.map((lb) => `| ${lb.employeeName ?? lb.employeeId} | ${lb.leaveTypeName ?? lb.leaveTypeId} | ${lb.openingBalance} | ${lb.earned} | ${lb.taken} | ${lb.pending} | ${lb.closing} |`),
            ];
            lines.push(formatPaginationFooter(res));
            return { content: [{ type: "text", text: truncate(lines.join("\n")) }] };
        }
