...
// Tool registration
// ---------------------------------------------------------------------------
export function registerHrisTools(server) {
    // ─── List Employees ───────────────────────────────────────────────────────
    server.registerTool("keka_list_employees", {
  ...
  - Get employees by IDs → employeeIds='emp1,emp2,emp3'`,
        ...
        try {
            ...
            if (params.employmentStatus)
                queryParams.employmentStatus = params.employmentStatus;
            if (params.employeeIds)
                queryParams.employeeIds = params.employeeIds;
            if (params.searchKey)
                queryParams.searchKey = params.searchKey;
            if (params.inProbation !== undefined)
                queryParams.inProbation = params.inProbation;
            if (params.inNoticePeriod !== undefined)
                queryParams.inNoticePeriod = params.inNoticePeriod;
            const res = await client.getPaginated("/hris/employees", queryParams);
            if (!res.succeeded) {
    ...
    // ─── Get Employee by ID ───────────────────────────────────────────────────
    server.registerTool("keka_get_employee", {
...
Returns: Complete employee profile including personal info, department, job title, manager, location, and employment details.`,
        ...
        try {
            const client = getKekaClient();
            const res = await client.getSimple(`/hris/employees/${params.employeeId}`);
    ...
    // ─── List Departments ─────────────────────────────────────────────────────
    server.registerTool("keka_list_departments", {
...
Returns: List of departments with IDs, names, parent department, and department lead.`,
        ...
        try {
            const client = getKekaClient();
            const res = await client.getPaginated("/hris/departments", {
                pageNumber: params.pageNumber,
                pageSize: params.pageSize,
            });
            if (!res.succeeded) {
            ...
            const lines = [
                ...
                `| Name | ID | Parent | Lead |`,
                `|---|---|---|---|`,
                ...res.data.map((d) => `| ${d.name} | ${d.id} | ${d.parentName ?? "—"} | ${d.leaderName ?? "—"} |`),
            ];
            lines.push(formatPaginationFooter(res));
            return { content: [{ type: "text", text: truncate(lines.join("\n")) }] };
        }
    ...
    // ─── List Job Titles ──────────────────────────────────────────────────────
    server.registerTool("keka_list_job_titles", {
...
Returns: List of job titles with their IDs and names.`,
        ...
        try {
            const client = getKekaClient();
            const res = await client.getPaginated("/hris/jobtitles", {
                pageNumber: params.pageNumber,
                pageSize: params.pageSize,
            });
            if (!res.succeeded) {
    ...
    // ─── List Groups ──────────────────────────────────────────────────────────
    server.registerTool("keka_list_groups", {
...
Returns: List of groups with IDs, names, and types.`,
        ...
        try {
            const client = getKekaClient();
            const res = await client.getPaginated("/hris/groups", {
                pageNumber: params.pageNumber,
                pageSize: params.pageSize,
            });
            if (!res.succeeded) {
