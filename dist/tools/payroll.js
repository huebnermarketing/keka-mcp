...
export function registerPayrollTools(server) {
    // ─── List Pay Groups ──────────────────────────────────────────────────────
    server.registerTool("keka_list_pay_groups", {
...
Use IDs with keka_list_salaries to filter by pay group.`,
        ...
        try {
            ...
            const res = await client.getPaginated("/payroll/paygroups", {
    ...
    // ─── List Pay Bands ───────────────────────────────────────────────────────
    server.registerTool("keka_list_pay_bands", {
...
Returns: Pay bands with ID, name, minimum and maximum salary amounts, and currency.`,
        ...
        try {
            const client = getKekaClient();
            const res = await client.getPaginated("/payroll/paybands", {
                pageNumber: params.pageNumber,
                pageSize: params.pageSize,
            });
            if (!res.succeeded) {
            ...
            const lines = [
                ...
                `|---|---|---|---|---|`,
                ...res.data.map((pb) => `| ${pb.name} | ${pb.id} | ${pb.minAmount ?? "—"} | ${pb.maxAmount ?? "—"} | ${pb.currency ?? "—"} |`),
            ];
            lines.push(formatPaginationFooter(res));
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
    ...
    // ─── List Salaries ────────────────────────────────────────────────────────
    server.registerTool("keka_list_salaries", {
...
Returns: Employee salaries with CTC (Cost to Company), pay group, currency, and effective date.`,
        ...
        try {
            ...
            if (params.payGroupIds)
                queryParams.paygroupIds = params.payGroupIds;
            if (params.employmentStatus)
                queryParams.employmentStatus = params.employmentStatus;
            const res = await client.getPaginated("/payroll/salaries", queryParams);
            if (!res.succeeded) {
                return { content: [{ type: "text", text: `Error: ${res.message}` }] };
            }
            if (!res.data.length) {
            ...
            const lines = [
                ...
                `|---|---|---|---|---|---|`,
                ...res.data.map((s) => `| ${s.employeeName ?? s.employeeId} | ${s.employeeNumber ?? "—"} | ` +
